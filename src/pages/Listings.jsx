...
import VisitLogModal from "../components/listings/VisitLogModal";
    ...
    <div className="max-w-5xl mx-auto px-5 py-10">
      ...
      {listings === null ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {listings.map((l, i) => (
            <ListingCard key={l.id} listing={l} index={i} isAdmin={isAdmin} onEdit={() => setFormTarget(l.id)} onApprove={approve} onReject={reject} onDelete={del} onLogVisit={() => setVisitTarget({ listingId: l.id, listingTitle: l.title })} />
          ))}
        </div>
      )}
      ...
      />
      <VisitLogModal target={visitTarget} onClose={() => setVisitTarget(null)} />
    </div>
  );
}


function ListingCard({ listing: l, index, isAdmin, onEdit, onApprove, onReject, onDelete, onLogVisit }) {
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {l.status === "active" && (
            <a href={`https://pinkcityproperties.com/listing/${l.id}`} target="_blank" rel="noreferrer" className="text-xs font-medium text-stone-600 border border-stone-200 rounded-full px-3 py-1.5 no-underline inline-flex items-center gap-1.5">
              <ExternalLink className="w-3 h-3" /> View Live
            </a>
          )}
          {l.status === "active" && (
            <button onClick={onLogVisit} className="text-xs font-medium text-jali border border-jali/30 rounded-full px-3 py-1.5 inline-flex items-center gap-1.5"><MapPin className="w-3 h-3" /> Log Visit</button>
          )}
          {l.type === "colony" && (
